...
def manpower_page():
 ...
 ### Setting choices   
    ...
    # if temp_project_info.query.get(1).mpd_worktime_daily != NULL:
 ...
 ### POST actions
    ...
    ### Rental mode
        ...
        if request.form.get('rental_mode_day'):
            ...
            print(request.form['rental_mode_day'])

            temp_project = temp_project_info.query.get(1)

            temp_project.mpd_worktime_daily = request.form['rental_mode_day']

            db.session.commit()

        if request.form.get('rental_mode_week'):

            print(request.form['rental_mode_week'])

            temp_project = temp_project_info.query.get(1)

            temp_project.mpd_worktime_weekly = request.form['rental_mode_weekly']
    ...
    ### Planner    
        ...
        if request.form.get('PlannerPlusBtnWorkdays'): 
            ...
            list_workdays = networkdays.Networkdays(datetime.strptime(planner_output['date_start'], '%Y-%m-%d'),
                                                 ...
                                                 datetime.strptime(planner_output['date_start'], '%Y-%m-%d') + timedelta(days=int(planner_output['workdays_stop'])-1))
 ...
 ### Create Gantt diagram
    ...
    # project_start = sorted([j.start for j in temp_planner_items])

    # project_stop = sorted([j.stop for j in temp_planner_items])

    # print(project_start[0])

    # print(project_stop[0])
    ...
    # index_ = ['Task', 'Resource', 'Start', 'Finish']
    ...
    fig = ff.create_gantt(df, 
                          ...
                          index_col='Resource',
                          jls_extract_var=False)#group_tasks=True,
